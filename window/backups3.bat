...
echo "start time %fullstamp%" >> E:\Crontab\log.txt

aws s3 sync E:\Backup s3://pushsale-backup/10.0.61.34/
echo Ok >> E:\Crontab\log.txt
...
set "HH=%dt:~8,2%" & set "Min=%dt:~10,2%" & set "Sec=%dt:~12,2%"
set "fullstamp=%YYYY%-%MM%-%DD%_%HH%-%Min%-%Sec%"
echo "End time %fullstamp%" >> E:\Crontab\log.txt
pause